        ...
        for i in tqdm(range(n_samples//batchsize)):
            ...
            for e in range(epochs):
                ...
                for j in range(batchsize//minibatch):
                    ...
                    loss = loss_function(out, pot_curr[j*minibatch:(j+1)*minibatch]) - 5*dual
                    if prints:
                        print("net loss, j="+str(j)+", loss="+str(loss_function(out, pot_curr[j*minibatch:(j+1)*minibatch]).item())+", "+str(dual.item()))
                    loss.backward()
                    self.optimizer.step()
